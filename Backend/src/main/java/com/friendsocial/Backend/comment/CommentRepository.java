...
public interface CommentRepository extends JpaRepository<Comment, Long> {
  ...
  @Query("SELECT c as comment, pr.username as username, pr.profilePic as profilePic, pr.id as profileId, po.id as postId FROM Comment c JOIN c.profile pr JOIN c.post po WHERE c.postId = ?1")
